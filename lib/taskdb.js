...
function createTask({ title, description, owner, assignee, priority, source, category, section, tags, due_date, project }) {
  const db = getDb();
  const stmt = db.prepare(`
    INSERT INTO tasks (title, description, owner, assignee, priority, source, category, section, tags, due_date, project)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
function getTask(id) {
  ...
  task.dependents = getDependents(id);
  return task;
}

function listTasks({ owner, assignee, status, priority, category, source, project, limit, offset, sort } = {}) {
  const db = getDb();
  const conditions = [];
  const params = [];
...
function updateTask(id, updates) {
  const db = getDb();
  const allowed = ['title', 'description', 'owner', 'assignee', 'status', 'priority', 'category', 'section', 'tags', 'due_date', 'project'];
  const sets = [];
  const params = [];
...
function addDependency(taskId, dependsOnId, type) {
  const db = getDb();
  if (taskId === dependsOnId) throw new Error('Task cannot depend on itself');
  const validTypes = ['blocks', 'related', 'subtask'];
  if (!validTypes.includes(type || 'blocks')) throw new Error('Invalid dependency type');
  db.prepare(`
    INSERT INTO task_dependencies (task_id, depends_on, type)
    VALUES (?, ?, ?)
  `).run(taskId, dependsOnId, type || 'blocks');
  return getDependencies(taskId);
}

function removeDependency(taskId, dependsOnId) {
  const db = getDb();
  db.prepare('DELETE FROM task_dependencies WHERE task_id = ? AND depends_on = ?').run(taskId, dependsOnId);
}

function getDependencies(taskId) {
  const db = getDb();
  return db.prepare(`
    SELECT d.id, d.task_id, d.depends_on, d.type, d.created_at,
