...
function vaultStats() {
  // Degrade gracefully when the vault base directory is absent (e.g. non-macOS
  // machines without the iCloud/Obsidian path), mirroring listNotes above.
  if (!fs.existsSync(VAULT_PATH)) return { totalNotes: 0, vaultPath: VAULT_PATH };
  let count = 0;
  function walk(dir) {
